...
const buttonVariants = cva(
  "group/button inline-flex shrink-0 items-center justify-center rounded-full border border-transparent bg-clip-padding text-sm font-medium whitespace-nowrap transition-colors outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 shadow-md hover:shadow-lg hover:shadow-indigo-500/20",
  {
    variants: {
      variant: {
        ...
        ghost:
          "hover:bg-muted hover:text-foreground aria-expanded:bg-muted aria-expanded:text-foreground dark:hover:bg-muted/50 shadow-none hover:shadow-none",
        destructive:
...
function Button({
  ...
  return (
    <MotionButton
      data-slot="button"
      whileHover={disableAnim ? undefined : { scale: 1.05 }}
      whileTap={disableAnim ? undefined : { scale: 0.9 }}
      transition={disableAnim ? undefined : { type: "spring", stiffness: 400, damping: 10 }}
